...
// Delete
node* Tree::RecDelete(node *root, int key){
    if(root == nullptr) return root;

    if(key < root->data){
        root->lchild = RecDelete(root->lchild, key);
    } else if(key > root->data){
        root->rchild = RecDelete(root->rchild, key);
    } else {
        // node found
        if(root->lchild == nullptr || root->rchild == nullptr){
            ...
            root->data = succ->data;
            root->rchild = RecDelete(root->rchild, succ->data);
        }
    }
